...
/**
 * Drain a teed SSE branch in the background to harvest usage, then record. Used for
 * streaming calls — the client branch is untouched (byte-for-byte passthrough).
 */
export function drainSseAndRecord(
  env: Env,
  ctx: ExecutionContext,
  body: ReadableStream<Uint8Array>,
  rec: Omit<ProviderCallRecord, keyof TokenUsage>,
): void {
    ...
    (async (): Promise<void> => {
      ...
      try {
        for (;;) {
          ...
          if (dump.length > 4_194_304) {
            dump = dump.slice(-1_048_576); // keep only the tail of runaway streams
          }
        }
      } catch {
        // interrupted stream — record what arrived
      }
      const usage = extractUsageFromSseDump(dump);
      await insertRow(env, { ...rec, ...usage }).catch((err: unknown) =>
        log.warn("usage_record_failed", {
          requestId: rec.requestId,
          provider: rec.provider,
          message: err instanceof Error ? err.message : "unknown",
